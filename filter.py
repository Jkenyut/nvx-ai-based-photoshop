...
def filter_gray(inputan_gambar, lokasi):
    image = cv2.imread(
        inputan_gambar, 0
    )  # membaca file dan 0 adalah pengubah gambar jadi gray
    lokasi = lokasi  # lokasi
    # lakukan pertama
    try:
        hasil_image = cv2.resize(
            image, (1920, 1080)
        )  # resize ukuran foto height,widht(1920,1080)
    # jika error ke expect
    except:
        hasil_image = image

    filename = cv2.imwrite(
        lokasi, hasil_image
    )  # save hasil pixel(nama file,hasil pixel)
    return filename
...
def filter_pixelize(inputan_gambar, lokasi):
    ...
    hasil_image = cv2.resize(
        hasil_image, (height_pixel, width_pixel), interpolation=cv2.INTER_LINEAR
    )
